...
if ! grep -qF "uses: actions/checkout@$APPROVED_CHECKOUT_SHA # v7.0.1" "$WORKFLOW"; then
  fail "actions/checkout must use the approved Node 24 commit"
fi
if ! grep -Eq '^          persist-credentials:[[:space:]]*false[[:space:]]*$' "$WORKFLOW"; then
...
if ! grep -Eq '^    timeout-minutes:[[:space:]]*[1-9][0-9]*[[:space:]]*$' "$WORKFLOW"; then
